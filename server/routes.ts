...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Business ads routes
  app.post("/api/businesses/:id/ads", isAuthenticated, async (req: any, res) => {
    try {
      const businessId = parseInt(req.params.id);
      const userId = req.user.claims.sub;
      ...
      const adData = insertBusinessAdSchema.parse({ ...req.body, businessId });
  ...
  app.post("/api/businesses/:id/loyalty-program", isAuthenticated, async (req: any, res) => {
    try {
      const businessId = parseInt(req.params.id);
      const userId = req.user.claims.sub;
      ...
      const programData = insertLoyaltyProgramSchema.parse({ ...req.body, businessId });
  ...
  app.post("/api/businesses/:id/join-loyalty", isAuthenticated, async (req: any, res) => {
    try {
      const businessId = parseInt(req.params.id);
      const userId = req.user.claims.sub;
      
      // Check if user already has an account
      const existingAccount = await storage.getUserLoyaltyAccount(userId, businessId);
      if (existingAccount) {
        return res.json(existingAccount);
      }
      
      const account = await storage.createUserLoyaltyAccount(userId, businessId);
      res.json(account);
    } catch (error: any) {
  ...
  app.get("/api/user/loyalty-accounts", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
  ...
  // Gift card routes
  app.post("/api/businesses/:id/gift-cards", isAuthenticated, async (req: any, res) => {
    try {
      const businessId = parseInt(req.params.id);
      const userId = req.user.claims.sub;
  ...
  app.get("/api/businesses/:id/is-following", isAuthenticated, async (req: any, res) => {
    try {
      const businessId = parseInt(req.params.id);
      const userId = req.user.claims.sub;
